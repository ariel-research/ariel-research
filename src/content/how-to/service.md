...
2. וודאו כי בקובץ זה ישנו משתנה בשם app המייצג את האפליקציה, וודאו כי ההרצה נעשית בצורה הבאה:
	```python
    app = create_app()  # or another command that creates a Flask application
	if __name__ == '__main__':
	    app.run(debug=True,host="0.0.0.0",port=<your-port>)
	```
4. התקינו את החבילה `gunicorn` כאשר הסביבה הוירטואלית שלכם פעילה (חזרו לשלב 5 בסעיף הקודם כדי להפעיל אותה). שימו לב: המערכת עובדת עם גירסה 22 ולא עובדת עדיין עם גרסאות מאוחרות יותר. 
	```
	pip install --force-reinstall --no-deps gunicorn==22
	```
5.  מעתה אתם הולכים להשתמש בשירות שלכם ע"י הפקודה `myservice`. על מנת שהפקודה תעבוד **חובה** להוסיף `sudo` בתחילת הפקודה.
6. הפעילו את השירות שלכם:
	```
	sudo myservice start
	```
    > בשלב זה קובץ בשם `app.sock` יווצר בתקית הפרויקט שלכם. גם כאן, הימענו מלגעת בו.
7. וודאו שהשירות פעיל:
	```
...
8. ניתן לבצע פעולות נוספות על השירות:
